var DocSetTokenCounter = function() {
  this.uniqueTokenCount = 0;
  this.frequencies = {};
  this.tokens = []; // optimization: unsorted array of indices into frequencies (because Object.keys() is slow)
  this.docsProcessed = 0;
}

DocSetTokenCounter.prototype.stopWords = {
...
DocSetTokenCounter.prototype.tokenize = function(text) {
...
DocSetTokenCounter.prototype.processDocument = function(doc) {
...
DocSetTokenCounter.prototype.getTopTokens = function(tokenLimit, Heap) {
  var frequencies = this.frequencies
    , compare     = function(k1, k2) { return frequencies[k1][1] - frequencies[k2][1]; }
    , tokens      = this.tokens.sort(compare) // in-place, to avoid a big Array allocation
    , firstTokens = tokens.slice(0, tokenLimit)
    , ret         = firstTokens.map(function(k) { return [ k, frequencies[k][0], frequencies[k][1] ]; });

  return ret;
};


module.exports = DocSetTokenCounter;
